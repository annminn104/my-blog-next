  ...
  return (
    <S.SliderItemWrap>
      ...
      <S.SliderText maxW='full'>
        <S.SliderItemTag as='h3'>{tag}</S.SliderItemTag>
        <S.SliderItemTitle as='h1'>{title}</S.SliderItemTitle>
        <S.SliderItemContent dangerouslySetInnerHTML={{ __html: content }} />
      </S.SliderText>
    </S.SliderItemWrap>
  )
}
...
function renderImageWithType(type: SlideTypeEnum, src: string, title: string): JSX.Element {
  const render: Record<SlideTypeEnum, JSX.Element> = {
    [SlideTypeEnum.Image]: (
      <S.SliderItemBackground as={Image} src={src} layout='fill' objectFit='cover' quality={100} alt={title} />
    ),
    [SlideTypeEnum.Video]: <></>
  }
